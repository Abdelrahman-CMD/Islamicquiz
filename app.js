...
function nameScreen(mode) {
  ...
  app.innerHTML = `
    <section class="screen panel">
      ...
      <form id="entryForm">
        ${joining ? `<div class="field"><label for="roomCode">Kamercode</label><input id="roomCode" maxlength="6" autocomplete="off" placeholder="Bijv. N00R24" value="${escapeHtml(state.roomCode)}"></div>` : ""}
    ...
    <section class="screen quiz-shell">
      ...
      <article class="question-card">
        ...
        <div class="answer-status">
          <span>${answered ? (revealed ? (answered.correct ? `Mā shā’ Allāh — +${answered.points} punten` : "Niet juist — maar wel iets geleerd") : "Antwoord opgeslagen. Neem rustig adem…") : `${room.players.filter(p => p.answered).length}/${room.players.filter(p => p.connected).length || room.players.length} hebben geantwoord`}</span>
      ...
      <div class="quiz-bottom">
        ${revealed ? `
          <article class="source-card reveal-card">
            ...
            ${room.isHost ? `<div style="margin-top:18px"><button class="button button-primary" id="nextBtn">${room.questionIndex === room.totalQuestions - 1 ? "Bekijk wat we leerden" : "Volgende vraag"} →</button></div>` : '<p style="margin-top:16px">De host gaat zo verder.</p>'}
          </article>` :
          `<article class="source-card"><span class="source-kicker">Na je antwoord</span><h3>Bron en uitleg</h3><p>Na iedere vraag verschijnt hier een korte uitleg met de gebruikte Qur’an- of hadithverwijzing.</p></article>`}
  ...
  app.innerHTML = `
    <section class="screen panel panel-wide" style="text-align:center">
      ...
      <div class="final-list">
        ${room.players.map((p, i) => `<div class="final-row"><strong>${i + 1}</strong><span style="text-align:left">${escapeHtml(p.nickname)}<small>${p.correctAnswers}/${room.totalQuestions} juist</small></span><strong>${p.score}</strong></div>`).join("")}
      </div>
